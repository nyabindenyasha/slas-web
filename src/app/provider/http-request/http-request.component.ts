...
export class HttpRequestComponent implements OnInit {
  loading: boolean;
  base: string = 'http://lecturer-appointment-system.herokuapp.com/';
  baseLocal: string = 'http://localhost:8080/';
  loader;
  ...
  public getLocal(url: string, func: any, error?: any): void {
    this.execute(this._http.get(this.baseLocal + url, this.getOptions()), func, error);
  }
  ...
  public postLocal(url: string, data: any, func: any, error?: any): void {
    this.execute(this._http.post(this.baseLocal + url, data, this.getOptions()), func, error);
  }
  ...
  public updateLocal(url: string, data: any, func: any, error?: any): void {
    this.execute(this._http.put(this.baseLocal + url, data, this.getOptions()), func, error);
  }
  ...
  public deleteLocal(url: string, func: any, error?: any): void {
    this.execute(this._http.delete(this.baseLocal + url, this.getOptions()), func, error);
  }
  ...
  private defaultError(e: Error, func: any) {
    ...
    if (status == 200) {
      swal.fire('', e['error'].text, 'warning');
      return;
    }

    if (status == 400) {
      swal.fire('', e['error'].message, 'warning');
      return;
    }

    if (status == 404) {
      swal.fire('', 'Requested service is not available', 'error');
      return;
    }

    if (status == 500) {
      swal.fire('', 'Requested service encountered an unexpected error', 'error');
      return;
    }

    swal.fire('', 'Unexpected error occurred', 'error');
  }
